...
const cancelBookingRoute = async (req, res) =>{
 ...
 await User.findOneAndUpdate({_id:userId}, {$pull:{Adventure:{_id:id}}}, {new:true});
...
//updte booking  Get-Route
  ...
  let foundAdventure = await User.findById({_id:userId}, {Adventure:{$elemMatch:{_id:id}}});
...
const postDelete =  async (req, res) => {
 ...
 await User.findOneAndUpdate({_id:userId}, {$pull:{Post:{_id:id}}}, {new:true});
...
const updatePost = async (req,res)=>{
  const id=req.params.id;
  let user = await User.findById({_id:req.user._id},  {Post:{$elemMatch:{_id:id}}}, {multiple:true} );
  ...
  if (req.isAuthenticated()) {
    ...
    User.findById(req.user._id, function (err, foundUser) {
      ...
      if (err) {
        ...
        res.render("dashboard", { user: foundUser.firstName, image:foundUser.image, Booking: foundUser, foundPost:foundUser.Post, fullYear: fullYear, message: req.flash("message") });
      }
    });
  ...
  if (req.isAuthenticated()) {
    ...
    User.findById(req.user._id, function (err, foundUser) {
      ...
      if (err) {
        ...
        res.render("myblogs", { user: foundUser.firstName, foundPost:foundUser.Post, fullYear: fullYear, message: req.flash("message") });
      }
    });
...
const  usersRoute = (req, res) => {
  ...
  if (req.isAuthenticated()) {
    ...
    User.findById(req.user._id, function (err, foundUser) {
      ...
      if (err) {
        ...
        res.render("userspage", { user: foundUser.firstName,  Booking: foundUser, foundPost:foundUser.Post, fullYear: fullYear, message: req.flash("message") });
      }
    });
  ...
  const registerRoute = (req,res)=> {
    ...
    res.render("Register",{user:req.user, message:req.flash("message"),error:req.flash("error")});
  }
